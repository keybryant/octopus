...
describe("ProjectStrip", () => {
  ...
  it("opens kanban drawer", () => {
    ...
    render(<ProjectStrip summary={currentProject()} onOpenKanban={onKanban} onOpenRequirements={() => {}} />)
    fireEvent.click(screen.getByRole("button", { name: /任务看板/ }))
    expect(onKanban).toHaveBeenCalledOnce()
  })

  it("opens requirements board", () => {
    const onRequirements = vi.fn()
    render(<ProjectStrip summary={currentProject()} onOpenKanban={() => {}} onOpenRequirements={onRequirements} />)
    fireEvent.click(screen.getByRole("button", { name: /需求看板/ }))
    expect(onRequirements).toHaveBeenCalledOnce()
  })
