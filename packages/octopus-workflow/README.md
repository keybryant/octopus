...
Agent 编排服务插件：主 agent 会话（工作台聊天）通过 15 个工具直接操作需求/项目/任务域，并为每个任务拉起独立 dsh 子会话执行。

## 能力

- **主会话工具**（15 个，注入 `ctx.tools`）：需求（`create_requirement` / `list_requirements` / `get_requirement` / `update_requirement`）、项目（`list_projects` / `get_project`）、任务（`list_tasks` / `get_task` / `create_tasks` / `update_task`）、会话编排（`send_to_task_session` / `task_session_status` / `stop_task_session` / `ask_task_session` / `task_session_log`）
- **事件驱动派发**：主 agent 不直接启动子会话。`create_tasks` 创建任务时可指定执行智能体（`agent` 字段，见 `list_agent_roles`）；任务状态置为 doing（执行中）时，`octopus-tasks` 派发 `octopus-tasks/task-status-changed` 事件，本插件监听该事件为任务**新建**独立执行会话（每次派发均新建，不复用旧会话）并启动对应智能体
...
- **任务子会话**：`TaskSessionManager` 为任务创建真实 AgentLoop 子会话（工作目录=项目工作区）；子 agent 完成后经 `report_task_status` 提交评审
...
> **降级行为说明**：当任一依赖域（requirements/tasks/projects）存储打开失败时，对应 store 服务不提供，octopus-workflow 插件整体不加载（主 agent 将看不到本插件的 15 个工具）。这与 spec「工具执行返回 session-unavailable」的字面差异已记录，正常态不可达。
