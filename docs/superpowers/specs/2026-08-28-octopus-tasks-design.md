...
3. `TasksDrawer.tsx`（壳新组件，仿 `RequirementsDrawer`）动态加载 tasks 模块；模块挂载时读取并**清空**载荷（消费一次），`requirementId` 传给 DecomposeDraftsModal 自动发起拆解
...
└── web/
    ...
    └── src/{index.tsx, api.ts, types.ts, status.ts, TaskBoard.tsx, DecomposeDraftsModal.tsx}
```
...
- Web：`TaskBoard` 拖拽 PATCH + 状态下拉兜底（组件测试）、`DecomposeDraftsModal` 载荷消费/草稿编辑/批量创建（api mock）、`api.test.ts`
...
| M3 | 前端看板 + 拖拽 + DecomposeDraftsModal + 模块注册 | 浏览器端到端：拆解→看板→拖拽→刷新不丢 |
...
4. 原生 HTML5 DnD 兼容性：桌面浏览器 OK，移动端无拖拽 → v1 兜底（卡片内状态下拉）已实现，避免无路径
