...
## Pre-flight corrections (controller rulings, 2026-08-28) �� binding over code blocks above

1. **T2**: `projectEvent` �� `projectEvents(state, ev): CapturedEvent[]`�������飬��Ϊ�գ���assistant/message ÿ�� text ����ÿ�� tool-call �������һ�� CapturedEvent��T 2 ���Զ��� `projectEvents(st, ev)` ���������� `toStreamEvent(ev)` ƥ�� expect �غɣ����� text �¼��Ӽ������� `expect(result.some(e => ...))`����
2. **T4**: SessionEntry ���� `approvalSeq: number`��approval id = `${sessionId}:a${entry.approvalSeq++}`��listener ֱ�� `Promise<ApprovalOutcome>`�������� next()����dispose/cancel ʱ�� `'cancelled'` settle ȫ�� pending��resume �� create ����ͬһ provider/model ���ˣ�`ensureLoaded` �� `persistence.load`/`resume` ���쳣ӳ�� `SESSION_NOT_FOUND`/`AGENT_LOOP_UNAVAILABLE`��ɾ�� `idsMatch`/`underTtl` �ȶ��� helper��ttl ����д�� list() �ڼ��жϣ���`captured` ����Ϊ null/await��
3. **T5**: `ApiDeps.manager.create` ȫ������ `CreateSessionInput`��types.ts����`POST /sessions/:id/messages` body ֧�ֿ�ѡ `answerQuestionId`��͸�� manager.send����ȫ��·����һ����ʵ�֣���ռλ��
4. **T6**: ���� `userQuestions` �š���`ctx.get("userQuestions")` ����ʱ try `registerProvider({ ask })`����ע�� �� catch ������ log����provider ��ÿ�� ask ӳ��Ϊ `{ id: `${sessionId}:q${n}`, question: `����`��questions[0].question��, options? }` д�� EventIndex��type "question"������ Promise��`POST /sessions/:id/messages { text, answerQuestionId }` ƥ�� pending �� resolve(answer text)���𸴺��� agent ������`send(id, text, answerQuestionId?)`��
5. **T7**: `datasource.ts` �� `createDefaultAgentClient` ��Ϊ�첽 thin re-export��ת�� agent-client.ts ͬ�������������Խ���ȫ�� `Response`���� `{ ok:boolean, status:number, json: async () => unknown } as never` �� fetch����
6. **T9**: ChatMessage `approval` ������� `data-testid="approval-{approvalId}"`�����ߺ�ť������ ChatPane ���� `decidedIds: Set<string>` ������
